...
import template from './balancer.html'


const controller = 'adminBalancerCtrl';
    ...
    constructor($scope, $state, RestCli) {
        ...
        $scope.createInterface = () => {
            ...
            if ($scope.interfaceModel.interface) {
                ...
                console.log(data)
                RestCli.post(`v1/balancers/${$state.params.id}/address`, data).then((disk) => {
                    $scope.reset()
                    $scope.update()
                })
            }
        ...
        $scope.removeInterface = (address) => {
            RestCli.delete(`v1/balancers/${$state.params.id}/address/${address.id}`).then((disk) => {
                $scope.reset()
                $scope.update()
            })
        }
        $scope.update = () => {
            RestCli.get(`v1/balancers/${$state.params.id}?populate=node`).then((nameserver) => {
            ...
            RestCli.get(`v1/balancers/${$state.params.id}/address?populate=interface`).then((address) => {
                console.log(address)
